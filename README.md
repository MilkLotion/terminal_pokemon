...
펫은 **항상 위에 뜨는 창이 아니다.** 일반 창과 같은 층에 두고, 자기 IDE 창 바로 위에만 올려 둔다.
그래서 다른 창이 그 위로 올라오면 **겹친 부분만 그 창에 가려진다** — 계산이 아니라 OS 가 처리한다.

```
A 창을 보는 중          →  [펫, A, B]
B 창을 클릭             →  [B, 펫, A]      B 와 겹친 부분만 가려진다
다시 A 를 클릭          →  A 가 펫 위로 올라오므로 곧바로 펫을 다시 올린다
```

마지막 줄의 "곧바로"가 관건이다. 폴링(0.4초)을 기다리면 그동안 펫이 창에 묻힌다. 그래서
**VS Code 확장이 포커스 변화를 파일에 적는 순간을 `fs.watch` 로 받아** 바로 올린다.
실측 반응 지연은 **중앙값 12ms**(최소 8ms)로, 60Hz 한 프레임보다 짧다.
...
`Cmd+Alt+K`(항상 보이기)는 "숨기지 않는다"는 뜻이지 "맨 위에 띄운다"는 뜻이 아니다.
다른 창이 위에 있으면 여전히 가려진다.
...
바꾼다. 실측에서 6~8% 확률로 걸렸다. 화면 밖으로 **완전히** 벗어난 창이 목록에 있으면
그 표본을 통째로 무시한다.
