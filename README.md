...
- **돌아다니고 존다** — 가끔 창 안을 걷고 두리번거린다. 입력이 5분 없으면 잠들고, 집어 들면 버둥거리고, 누르면 반응한다
- **CLI 상태에 반응한다** — CLI 안에서 띄우면 훅이 알려 주는 상태에 맞는 동작을 재생한다

| 상태 | 언제 | 동작 (PMD) |
|---|---|---|
| `running` | 프롬프트 전송 · 도구 실행 | 걷기 |
| `waiting` | 승인 대기 | 두리번 |
| `waving` | 세션 시작 · 턴 끝 | 인사 한 번 |
| `failed` | 도구 · 턴 실패 | 쓰러짐 |
| `idle` | 그 밖 | 대기 · 산책 · 수면 |

훅이 없는 CLI 나 일반 터미널에서는 상태 동작 없이 산책 · 수면 · 만지기 반응만 한다.
