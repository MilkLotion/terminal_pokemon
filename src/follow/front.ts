...
// lib/follow.js 를 타입만 붙여 옮긴 것 — export 이름·동작은 그대로 (cli/run.js 가 TERM_PROGRAM_APPS 를 이름으로 부른다)
//
...
import type { HelperInfo, HelperWindow, HostInfo, SelfMark, StateRecord, WindowRecord } from "./types";
...
export const isKnownTerminal = (app: unknown): boolean => KNOWN_TERMINAL_APPS.has(String(app || "").toLowerCase());

// TERM_PROGRAM 값 → 헬퍼의 app 이름. 조상으로 창 주인을 못 찾을 때(tmux 등)의 대비책 (cli/run.js anchorApp)
export const TERM_PROGRAM_APPS: Readonly<Record<string, string>> = {
  vscode: "Code",
  ghostty: "Ghostty",
  "iTerm.app": "iTerm2",
...
const isSelf = (w: HelperWindow, self: SelfMark | null | undefined): boolean =>
...
export function frontWindow(
  info: HelperInfo | null | undefined,
  windows: HelperWindow[] | null | undefined,
  self?: SelfMark | null,
): HelperWindow | null {
  if (!info || !Array.isArray(windows)) return null;
  const frontPid = Number(info.frontPid) || 0;
  let front: HelperWindow | null = null;
  if (frontPid) {
...
//   windowRecords  확장이 적은 창 기록 (살아 있는 것만, state.ts readWindowRecords)
...
export function hostOf(
  front: HelperWindow | null | undefined,
  windowRecords: WindowRecord[] | null | undefined,
  stateRecords: StateRecord[] | null | undefined,
): HostInfo | null {
