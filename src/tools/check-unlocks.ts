...
// 전설·환상·울트라비스트 종의 입수 경로는 스펙에서 아직 정하지 않았다 (docs/specs/s5.md "이후 획득").
...
process.stdout.write(`진화 규칙의 출발 종이 규칙·알 어디에도 없는 종 ${r.brokenFrom.length} (출발 종이 전설·환상·울트라비스트라 해금 규칙이 없다)\n`);
