...
// 출력: {"frontmost":"Code","frontPid":2108,"windows":[{"app":"Code","pid":2108,"id":12345,"x":0,"y":30,"w":2560,"h":1324}]}
// frontPid 는 맨 앞 앱의 프로세스 번호 — 이름(frontmost)만으로는 펫끼리(전부 Electron) 가를 수 없어 동반자가 이걸로 맨 앞 창을 고른다
...
let frontmost = frontApp?.localizedName ?? ""
...
print("{\"frontmost\":\"\(frontmost)\",\"frontPid\":\(frontPid),\"windows\":[\(items.joined(separator: ","))]}")
