...
| 일반 로그인 아이디 (2026-09-27) | 이메일 + 비밀번호다. | 선택지 "이메일 + 비밀번호" |
| 소셜 로그인 (2026-09-27) | Google과 GitHub다. | 선택지 "Google, GitHub" |
...
2026-09-27 사용자 지시로 추가했다. 출처는 [출처와 사용자 결정](#출처와-사용자-결정)의 2026-09-27 행이다. 사용자 결정 외의 내용은 `[스펙 미확정]` 제안이다.
...
- 로그인 방법은 이메일+비밀번호, Google, GitHub 세 가지다.
- 본인인증(휴대폰·실명 확인)은 하지 않는다. 가입 확인 메일도 보내지 않는다.
...
Supabase는 익명 사용자에 이메일이나 소셜 계정을 연결해 같은 사용자 ID를 유지할 수 있다. 그런데 익명 사용자에 비밀번호를 붙이려면 이메일 확인이 먼저 필요하다. 이 설계는 이메일 확인을 하지 않는다.
그래서 익명 계정을 바꾸지 않는다. 로그인하면 정식 계정으로 세션을 바꾼다. 남은 익명 사용자는 정리 작업이 지운다.

이 방식이 안전하려면 세션을 바꿀 때 걸린 교환이 없어야 한다.

- 내 채널이 `open`·`joined`이거나 로컬 `trade.pending`이 있으면 로그인·로그아웃·계정 삭제를 막는다. 화면은 "교환을 끝낸 뒤 다시 시도" 계열 문구를 보인다.
...
| 동작 | 제안 |
|---|---|
| 가입 | `signUp({ email, password })`. 프로젝트 설정의 이메일 확인(Confirm email)을 끈다. 가입하면 바로 로그인된다 |
| 로그인 | `signInWithPassword({ email, password })` |
| 비밀번호 규칙 | 8자 이상. Supabase 프로젝트 설정으로 강제한다 |
| 비밀번호 재설정 | 재설정 메일에 링크 대신 6자리 코드를 보낸다. 앱에서 코드와 새 비밀번호를 받는다. `verifyOtp({ email, token, type: 'recovery' })` 뒤 `updateUser({ password })`를 호출한다. 데스크톱 앱이라 메일 링크로 앱을 여는 흐름을 쓰지 않는다 |
| 메일 발송 | Supabase 기본 메일은 발송 대상과 발송 수가 제한된다고 알고 있다. 문서로는 확인하지 않았다(A-05). 제한이 맞으면 외부 SMTP를 연결한다. 서비스는 정하지 않았다 |

가입 확인을 하지 않으므로 남의 이메일로도 가입할 수 있다. 이 경우 재설정 메일은 실제 주인에게 간다. 가입한 사람은 비밀번호를 잊으면 계정을 되찾을 수 없다. 친구 교환만 있는 지금 범위에서는 이 위험을 받아들인다.
...
2. `signInWithOAuth({ provider, options: { redirectTo, skipBrowserRedirect: true } })`로 로그인 주소를 받는다.
3. `shell.openExternal`로 기본 브라우저를 연다. 사용자는 브라우저에서 로그인한다.
...
- 준비물: Google Cloud의 OAuth 클라이언트와 동의 화면, GitHub OAuth App. 두 곳의 콜백 주소는 `https://<프로젝트>.supabase.co/auth/v1/callback`이다. 등록과 비밀 값 입력은 사용자가 대시보드에서 한다.
- 요청 범위는 이메일과 기본 프로필만이다. 저장하는 값은 Supabase가 `auth.users`에 두는 값뿐이다. 앱 테이블에 이름·사진을 따로 두지 않는다.
- 같은 이메일의 이메일 계정과 소셜 계정이 있을 때 Supabase의 자동 연결 동작은 구현 때 확인한다. 확인하지 않은 이메일 계정은 자동 연결 대상이 아닐 수 있다. `[스펙 미확정]`
...
| 계정 삭제 | 설정 화면에서 한다. 서비스 역할 키가 필요하므로 Edge Function `delete-account`가 처리한다. 함수는 요청한 사용자의 JWT로 본인을 확인한 뒤 `auth.admin.deleteUser`를 호출한다 |
| 삭제와 교환 기록 | 채널의 `host`·`guest` 외래 키를 `on delete set null`로 둔다. 상대가 아직 반영하지 않은 `done` 채널을 남기기 위해서다. [테이블](#테이블)의 `host` 정의도 이렇게 바꾼다 |

#### 세션 저장

- 익명 세션과 정식 세션은 같은 곳에 저장한다. 메인 프로세스의 `safeStorage` 암호화 파일이다.
- 렌더러에는 로그인 상태, 로그인 방법, 이메일(표시용)만 넘긴다. 토큰은 넘기지 않는다.
...
- 설정 모달에 `계정` 행을 둔다. 로그인 전에는 `로그인`, 로그인 뒤에는 로그인 방법과 이메일을 보인다.
- 계정 화면은 가이드북처럼 설정 모달 안에서 바뀌고 `‹ 설정`으로 돌아간다.
- 로그인 화면: `Google로 계속`, `GitHub로 계속`, 이메일·비밀번호 칸, `로그인`, `가입`, `비밀번호 재설정`.
- 로그인 뒤: `로그아웃`, `계정 삭제`(확인 대화상자).
- 교환 중이라 막힌 상태의 안내.
...
| 상황 | 화면 |
|---|---|
| 이메일 또는 비밀번호가 틀림 | 로그인 칸 아래 안내. 어느 쪽이 틀렸는지 나누지 않는다 |
| 이미 가입한 이메일 | 가입 칸 아래 안내 |
| 비밀번호 규칙 미달 | 가입 칸 아래 안내 |
| 재설정 코드가 틀리거나 지남 | 재설정 칸 아래 안내 |
...
| 위험 | 대응 |
|---|---|
| 이메일 수집 | 공개 배포 전에 개인정보 처리 안내를 준비한다. 수집 항목은 이메일과 소셜 계정의 기본 프로필이다 |
| 외부 SMTP 필요 | 비밀번호 재설정을 쓰려면 필요하다. 서비스와 비용은 정하지 않았다 |
| 가입 확인 없음 | 남의 이메일로 가입할 수 있다. 위 절의 대응을 따른다 |
| OAuth 앱 등록 | Google 동의 화면이 "테스트" 상태면 등록한 사용자만 로그인한다. 공개 전에 "게시" 상태로 바꾼다 |
| 세션 전환 중 교환 | 걸린 교환이 있으면 세션 전환을 막는다 |

#### 수용 조건 (구현할 때 검사한다)

1. 로그인하지 않아도 교환이 된다.
2. 이메일로 가입하면 확인 메일 없이 바로 로그인된다.
3. 비밀번호 재설정 코드로 새 비밀번호를 정하고 로그인한다.
4. Google·GitHub 로그인 뒤 브라우저에서 앱으로 돌아와 로그인 상태가 된다.
5. 다른 PC에서 같은 계정으로 로그인한다.
6. 교환이 걸려 있으면 로그인·로그아웃·계정 삭제가 막힌다.
7. 계정을 삭제해도 상대의 반영하지 않은 교환은 반영된다.
8. 렌더러와 저장소에 토큰과 서비스 역할 키가 없다.
