...
- 관리 창의 탭은 파티 / 도감 / 상점 / 가방이다. 업적창과 설정은 헤더 아이콘이다. 파티 탭의 박스 목록 안에 돌보미집을 둔다. [근거](work/s5-design-system-v2/plan.md#관리-창의-탭과-헤더-구성)
...
재개 방향: 저해상도 와이어프레임을 이어서 작성한다. 두 번째 업적은 사용자가 구상을 정할 때까지 보류한다. 가격·성장 곡선·확률은 밸런스 설계로 남긴다.

재개 요청 예시:

> docs/progress.md의 다른 세션에서 재개 안내를 읽고 전체 설계를 이어가자. 두 번째 파티 확장 업적은 미정으로 보류해. 저해상도 와이어프레임부터 진행해줘. 아직 구현하지 마.
...
| 화면·모듈 | 시나리오별 담당 영역 초안을 작성했다. 관리 창의 탭·헤더 구성을 확정했다. | 박스 목록 안 돌보미집의 세부 배치, 저해상도 와이어프레임, 컴포넌트 계약, 모듈 경계, Figma 탭 반영 |
...
4. 관리 창의 탭·헤더 구성은 확정했다. 저해상도 와이어프레임을 만든다. 흐름 ID를 화면과 수용 검사에 연결한다.
...
flowchart LR
  ...
  A10 --> C10["Figma v2 시각 기준 00~05<br/>탭: 파티·도감·상점·가방<br/>헤더: 포인트·업적창·설정 아이콘<br/>파티 탭 안 박스 목록, 그 안 돌보미집<br/>우클릭·트레이 메뉴 구성"]
  A10 --> O10["박스 목록 안 돌보미집 세부 배치<br/>와이어프레임·컴포넌트 계약<br/>Figma 열린 문제 12개, 탭 구성 반영<br/>창 크기·배율 정책"]
