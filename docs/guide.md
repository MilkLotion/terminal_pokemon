...
- **첫 실행** — 저장(`~/.claude/pokebuddy/save.json`)이 없거나 파티가 비었으면 포켓몬 선택 창이 뜬다(스타터 29종 — 세대별 3종 + 피카츄·이브이). 고르면 그 종으로 펫이 뜨고, 창을 닫으면 시작하지 않는다. 동반자 실행 명령은 `pokebuddy companion` 하나이며, 포켓몬은 이 선택 창에서 고른다. 성격은 무작위다. 저장이 있으면 저장된 파티를 복원한다.
...
- 원본은 `assets/logo/src/logo.txt`(32×32, 48px 이상용)와 `logo.small.txt`(16×16, 16·32px 용). 위쪽에 팔레트(`기호 = #RRGGBB`, `.` 은 투명),
  아래에 문자 격자라 텍스트 편집기로 고친다. 현재 시안은 터미널 창 모서리에 걸친 빨강·흰색 몬스터볼이다.
