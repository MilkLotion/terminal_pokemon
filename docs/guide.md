...
이 가격은 조정 가능한 초기값이다. 보상 규칙은 [계획과 규칙](plan-s4.md)에 있다.
...
> **S3(2026-09-17)에서 적립을 다시 시작했다.** 무대 통합(S2) 동안 멈췄던 친밀도·기분·배고픔 적립을 `src/state/`가 되살렸다. 확정값은 다음과 같다 — 켜 두기는 시간당 친밀도 +6, 토큰은 1천당 +1(토큰을 못 읽는 작업은 시간당 +12), 마리별 하루 친밀도 상한은 200, 포인트는 토큰 1만당 +1이고 하루 상한은 50, 배고픔은 시간당 +10이고 밥을 주면 −40, 활동성이 +1이면 걷기가 20% 늘어난다. 이 값은 [design.md](design.md)와 [S3 검토 기록](review-s3.md)을 따른다. 아래 표의 원천 종류(켜 두기·일한 시간·턴 완료·밥·놀기·찌르기)는 1판부터 그대로 쓴다. 밥·놀기·찌르기의 정확한 양과 하루 상한은 위 확정값 이후 다시 확인하지 않았다 [확인 필요].
> 우클릭의 밥 주기·놀아주기는 S3 에서 돌아왔다.

- **첫 실행** — 저장(`~/.claude/pokebuddy/save.json`)이 없거나 파티가 비었으면 포켓몬 선택 창이 뜬다(스타터 29종 — 세대별 3종 + 피카츄·이브이). 고르면 그 종으로 펫이 뜬다. 창을 닫으면 시작하지 않는다. 동반자 실행 명령은 `pokebuddy companion` 하나다. 포켓몬은 이 선택 창에서 고른다. 성격은 무작위다. 저장이 있으면 저장된 파티를 복원한다.
...
- **친밀도 원천** — 원천 종류는 1판부터 그대로 쓴다. 아래 이벤트별 양은 1판 규칙이다 [확인 필요]. S3 확정값(2026-09-17)은 마리별 하루 상한을 200으로 정했다. 상한을 넘으면 기분만 오른다.
...
- **기분** — 범위는 0~100이고 시작값은 60이다. 밥 +10, 놀기 +5, 방치 −1/시간(보이는 동안만), 에이전트 실패 −5(시간당 −10까지)의 증감이 있다. 메뉴에는 숫자 대신 말로 표시한다(최고·좋음·보통·시들·우울). 친밀도는 절대 줄지 않는다
- **하루** — 날짜가 바뀌면 상한이 리셋된다. 어제 교감(밥·놀기·찌르기)했으면 연속(streak)이 1 늘어난다. 오늘 첫 교감에는 포인트 5×연속(상한 30)을 받는다. 포인트는 [S4 상점](#s4-상점과-진화)에서 칸·새 종·민트·먹이·에버스톤·색 구매에 쓴다
- **단계** — 친밀도 500 과 1500 에서 진화를 준비한다. 실제 진화는 [S4 상점과 진화](#s4-상점과-진화)에서 구현했다
- **저장을 쓰는 펫은 하나** — `save.lock` 을 먼저 잡은 펫이 쓴다(`src/save/writer.ts`). 독립 펫이 살아 있으면 창 펫은 잠금을 내준다(독립 펫 우선, 10초마다 확인). 나머지 펫은 읽기 전용으로 폴더를 감시해 다시 읽는다. 놓은 자리·보이기 같은 요청은 `mailbox/` 폴더에 파일로 넣는다. 그 요청은 쓰는 펫이 처리한다(`src/save/mailbox.ts`). 1판은 매 10초 시계로 쌓이는 시간을 값이 바뀔 때만 파일에 썼다(10분·1분 단위)
- **토큰** — Claude Code 는 상태 훅이 턴 끝(`Stop`)마다 대화 기록(`transcript_path`)에서 새 응답의 토큰(입력·출력·캐시 읽기·캐시 쓰기)만 읽는다. 그 값을 세션 기록에 누적한다(`usage` · `usageOffset`). 세션 시작(재개 포함) 시점의 크기를 기준으로 잡는다. 그래서 옛 대화는 세지 않는다. 아직 쓰는 중인 마지막 줄은 다음에 읽는다. 증분 읽기는 `src/agents/usage.ts` 에 있다. S3 에서 이 토큰이 친밀도·포인트의 원천이 되었다. 무대는 아직 세지 않는다
- **시각은 전부 ms** (`Date.now()`). 저장 스키마(`src/shared/types.ts`)는 `docs/design.md` 의 JSON 보다 마리별 `daily` 에 `date · feeds · plays · presence · work · turns` 가 더 있다. 파손된 저장은 `save.json.bak` 으로 옮긴다. 그리고 새로 시작한다. 이것이 진행을 잃는 유일한 경로다. 그래서 stderr 에 알린다
...
- 독립 펫은 트레이 아이콘을 하나 둔다. 파티 첫 마리의 서 있는 그림 첫 프레임을 잘라 쓴다(그림이 없으면 흰 네모). 툴팁은 `pokebuddy · <이름>` 이다. 메뉴 구성은 다음과 같다 — 이름(누를 수 없는 한 줄), 잠시 숨기기/다시 보이기, 고스트 모드(마우스 클릭을 무시, 체크 표시), 설정 파일 열기, 종료. Windows 는 왼쪽 클릭에도 메뉴가 열린다
- 모든 펫에 마리별 우클릭 메뉴가 있다. 메뉴 구성은 `이름 · 성격`(누를 수 없는 한 줄. 세션 펫은 성격이 없어 이름만 나온다), 잠시 숨기기/다시 보이기, 종료다. 그림 위에서만 열린다. 숨기기·종료는 그 마리가 아니라 무대 전체(그 펫 프로세스)에 걸린다. 끝내기 메뉴 항목은 어느 펫이든 "종료" 라고 쓴다. 호칭(펫·동반자)은 메뉴에 쓰지 않는다. 동사만 쓴다. 이름은 별명이 있으면 별명을 쓴다. 별명이 없으면 종의 화면 이름을 쓴다
- 고스트 모드는 설정 항목이다. 그래서 우클릭 메뉴에는 없다. 켜면 우클릭이 안 되어 끌 수 없기 때문이다. 설정창이 생기기 전까지는 트레이에서 끈다. 트레이가 없는 펫은 단축키로 끈다. 밥 주기·놀아주기는 S3 에서 우클릭에 돌아왔다
- 독립 펫의 고스트 모드(클릭 통과) 토글은 저장하지 않는다. 전역 설정이므로 세션 펫의 다음 실행까지 번진다. 모드별 설정은 설정창(다음 단계)에서 다룬다
...
무대에 보이는 동안 마리마다 종·성격에 따라 친밀도가 오른다. 연결된 훅이 토큰을 읽으면 토큰을 센다. 그렇지 않으면 작업 시간을 센다. 친밀도는 줄지 않는다. 배고픔과 기분만 오르내린다. `pokebuddy status`에서 상태를 확인할 수 있다. 초기 수치와 검증 범위는 [S3 검토 기록](review-s3.md)에 있다.
