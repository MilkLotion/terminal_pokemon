...
| `src/notify` | 신규 | 알림 배너 줄 세우기, 같은 상태 한 번 규칙, 표시 순서 | 상태 판정 | SC-04, 05, 06, 10 |
| `src/main` | 조정 | 창, 트레이, 우클릭 메뉴, 명령 수신 | 게임 규칙 계산, 배너 순서 | 전체 |
...
2026-09-23 결정: 요청 ID와 결과는 저장의 `tx` 영역에 둔다. 최근 200건 또는 24시간 중 큰 쪽을 남긴다. 거래는 한 번에 하나만 처리한다. 들어온 순서대로 줄을 세운다. 창이 여러 개여도 저장 쓰기는 주 프로세스 하나가 한다. 근거는 [저장과 거래의 남은 값 확정](../work/s5-design-system-v2/plan.md#저장과-거래의-남은-값-확정)을 따른다.
...
2026-09-23 결정: 주기 저장은 30초마다 한다. 기존 저장의 `inventory`에는 먹이 재고가 없다. 유일한 키는 `shiny:<개체 식별자>`이며 이로치로 바꿀 권리를 산 기록이다. 모습이 바뀌는 약을 1회 소모품으로 정했으므로 이 기록은 `bag`으로 옮기지 않고 `legacy`에 보존한다. 이미 이로치인 개체는 `pets[].shiny`로 그대로 남는다. 저장이 3회 이어서 실패하면 관리 창의 상태 안내에 남긴다. 조작은 막지 않는다. 근거는 [저장과 거래의 남은 값 확정](../work/s5-design-system-v2/plan.md#저장과-거래의-남은-값-확정)을 따른다.
...
2026-09-23 확인: `party.summon`과 `party.dismiss`는 코드에 없다. 현재 이름은 `party.show`와 `party.hide`이며 새 계약도 같은 이름을 쓴다. 폐기할 명령이 없다.
2026-09-23 확인: VS Code 확장은 게임 명령을 보내지 않는다. 창과 터미널 상태를 `windows/` 아래 JSON으로 쓸 뿐이다. 명령 계약을 바꿔도 확장은 영향을 받지 않는다. 근거는 [저장과 거래의 남은 값 확정](../work/s5-design-system-v2/plan.md#저장과-거래의-남은-값-확정)을 따른다.
