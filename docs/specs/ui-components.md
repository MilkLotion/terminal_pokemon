...
근거: [0.2 · 와이어프레임(구 07)](https://www.figma.com/design/MA3K41Y6omAi5mRu6YDFly/pokebuddy?node-id=252-121)의 WF-01~WF-09와 상태 예시, Figma `00`~`04` 자산의 읽기 전용 확인, [작업 기록](../work/s5-design-system-v2/plan.md#ui-컴포넌트-계약-추출-설계).
...
| 규칙 | 내용 | 근거 |
|---|---|---|
| 선택 표현 | 카드·목록 항목은 옅은 배경만 쓴다. 왼쪽 세로 막대는 2026-09-26에 뺐다([화면 도트 스타일](../work/ui-pixel-style/record.md#재설계--기존-스타일-위의-galmuri)). 메인 탭은 옅은 배경과 밑줄이다. 필터 칩은 진한 채움과 흰 글자다. 두 칸 전환은 회색 틀 안의 흰 칸이다. 헤더 아이콘은 모달이 열린 동안 진한 배경이다. | [현재 설계](../design.md#s5-디자인-시스템-v2) |
...
| 글꼴 | 모든 글자는 Galmuri다. 크기는 24·15·12·10px 네 단계만 쓴다. 카드의 이름·설명이 칸보다 길면 한 줄로 두고 끝을 말줄임한다. | [화면 도트 스타일](../work/ui-pixel-style/record.md#재설계--기존-스타일-위의-galmuri) |
...
템플릿: 2026-09-24 `Shop Layout` `379:1757`의 알 분류에 상품 `태고의돌` `405:2539`을 더했다. 화석 전용 알이며 200P다. 같은 날 진화 분류와 `section/진화` `404:2309`를 더했다. 진화용 도구 24종을 모두 담는다. 도구 절에 있던 진화의돌 하나는 뺐다. 근거는 [진화용 도구 가격과 노출](../work/s5-design-system-v2/plan.md#진화용-도구-가격과-노출)을 따른다.
...
개체 상세의 경험치는 이 막대를 쓰지 않는다. 레벨 옆에 짧은 막대와 `Lv.9까지 20%`를 붙인다. 근거는 [빈 화면 제안 검토](../work/s5-design-system-v2/plan.md#빈-화면-제안-검토)를 따른다.
...
Figma: `Box Toolbar` `337:284`. 버튼과 입력칸은 높이 32다. `Show Sort`로 정렬 칸을 끈다.
...
Figma: `Egg Slot` `334:238`(`Preparing|Ready|Empty|Selected`), `Daycare Strip` `334:239`.
...
Figma: `Species Card` `336:299`(`Obtained|Unlocked|Locked|Selected`). 이로치는 `Show Shiny`로 켠다.
...
Figma: `Quantity Stepper` `332:187`. 버튼은 노출한 인스턴스라 최소·최대에서 `State=Disabled`로 둔다. `Show Max`, `Show Hint`를 둔다.
...
Figma: `Filter Chip` `331:130`(`Default|Selected`). 두 상태 모두 높이 28이다.
...
부화·진화 배너도 같은 구성이다. 부화는 `부화 준비 완료`와 알, 진화는 `진화 가능`과 개체를 보여준다. `바로가기`는 각각 돌보미집과 개체 상세를 연다. 결과 포켓몬과 결과 종은 보여주지 않는다. 근거는 [빈 화면 제안 검토](../work/s5-design-system-v2/plan.md#빈-화면-제안-검토)를 따른다.
표시 시간 8초와 주 화면 기준은 2026-09-25 구현에서 정했다. [알림 배너의 설계](../work/game-runtime/record.md#알림-배너의-설계)를 따른다. 구현은 `src/renderer/banner.html` 이다.
Figma: `Notification Banner` `338:732`(`Type=Achievement|Hatch|Evolution`).

### C-20 배고픔 말풍선

역할: 보이는 포켓몬이 배고픔 알림 구간에 들어갈 때 한 번 표시한다.
`[스펙 미확정]` 문구, 위치, 표시 시간.
Figma: `Speech Bubble` `338:733`.
...
Figma: `Context Menu` `338:738`과 항목 `Menu Item` `337:319`(`Default|Hover|Disabled`, `Show Hint`). 밥 주기 불가는 `feed`를 Disabled로 두고 남은 시간을 붙인다.

### C-22 빈 상태 줄

역할: 목록이 비었거나 검색 결과가 없을 때 한 줄로 알린다.
사용: 빈 박스(`보관 개체 없음`), 도감 검색(`검색 결과 없음`).
Figma: `Empty State` `332:199`.

### C-23 코치마크 말풍선

역할: 튜토리얼의 한 단계를 대상 옆에서 알려준다.
내용: 단계 수(`튜토리얼 · 상점 1 / 2`), 제목, 설명, `다음`, `✕`.
...
사용: 개체 상세의 밥 주기·놀아주기, 알 돌봄의 쓰다듬기·노래 들려주기.
...
상태: 기본, 선택. 선택은 옅은 배경으로 보여준다. 선택 막대와 컬러 테두리를 쓰지 않는다.
사용: 첫 선택 화면의 후보 29개.
...
2. C-01~C-25를 Figma에 만들었다. 다음은 템플릿 수정이다. 기존 V2-01~V2-12 문제와 함께 처리한다.
3. 모듈 책임과 구현 컴포넌트 구조는 별도로 정한다.
4. 2026-09-23 시나리오 빈 화면 와이어프레임에서 반복 후보가 생겼다. 코치마크 말풍선, 돌봄 타일(밥 주기·놀아주기·쓰다듬기·노래 들려주기), 미리보기 상자, 진화 전후 비교다. 코치마크 말풍선, 돌봄 타일, 정보 상자는 C-23~C-25로 더했다. 진화 전후 비교는 한 화면에서만 쓰여 계약에 넣지 않았다. 근거는 [작업 기록](../work/s5-design-system-v2/plan.md#시나리오-빈-화면-와이어프레임)을 따른다.
