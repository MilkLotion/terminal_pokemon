...
| 명령 | 결과 |
|---|---|
| `npm run logo:build` | 통과. TypeScript 빌드 후 PNG 8종, SVG, ICO, ICNS 11개를 생성했다. |
...
- 잔여 이전 캐릭터 픽셀이 첫 시안에서 몬스터볼과 붙었다. 상태: 해결. 해당 픽셀을 원본 맵에서 제거하고 다시 생성했다.
- 16px 맵은 작은 크기용으로 단순화했다. 상태: 해결.
- 큰 래스터 결과에서 볼이 타원·반쪽처럼 보였다. 상태: 해결. 32×32 원본의 볼을 약 18×17 대칭 계단 형태로 확대하고 다시 생성했다.
- 사용자가 볼을 더 크게 요청했다. 상태: 해결. 16px 맵도 큰 단순형으로 갱신했다.
- 터미널 창이 길다는 피드백이 있었다. 상태: 해결. 본문 폭을 20px에서 16px로 줄이고 몬스터볼 겹침 구도를 다시 맞췄다.
- 런타임 코드 변경은 필요하지 않다. 상태: 확인.

## 최종 판정

기존 픽셀 로고 산출물을 새 공식 SVG 로고로 교체했다. `npm run logo:build`가 공식 SVG를 기준으로 PNG 8종, SVG, ICO, ICNS, VS Code PNG를 갱신한다. 열린 findings는 없다.
...
- `src/main/tray.ts`가 첫 포켓몬 Idle 스프라이트를 자르지 않고 공식 `logo-256.png`를 사용한다.
...
- 발견: `qlmanage`가 1024px PNG 캔버스 안에 256px SVG를 배치해 시스템 아이콘이 작은 흰 사각형처럼 보였다.
