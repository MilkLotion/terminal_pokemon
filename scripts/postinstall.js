...
//    그러면 사용자가 처음 !termimon 을 칠 때 펫이 뜨기 전에 100MB 다운로드가 끼어든다.
//    여기서 실패해도(오프라인 등) 설치는 성공시키고, termimon setup 이 다시 받는다.
...
try {
  try {
    ...
    process.stdout.write("Electron 을 미리 받지 못함 — 네트워크가 되는 곳에서 termimon setup 을 실행하면 받는다\n");
  }
  ...
  process.stdout.write("termimon 설치 완료 — 처음 한 번: termimon setup\n");
} catch {
  // 설치 후 안내 실패는 무시
}
