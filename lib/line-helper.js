...
function createLineHelper(cmd, args, { timeoutMs = 2000, startTimeoutMs = 20000 } = {}) {
  let proc = null; // { child, buf, ready, pending: { cb, timer } }
  // 답 한 줄 못 하고 끝난 횟수 — 헬퍼가 망가졌으면(컴파일 실패 등) 폴링마다 다시 띄우지 않게 간격을 벌린다
  let failures = 0;
  let retryAt = 0;

  const settle = (p, err, line) => {
    const q = p.pending;
    if (!q) return;
    p.pending = null;
  ...
  const start = () => {
    ...
    const p = { child, buf: "", ready: false, pending: null };
...
module.exports = { createLineHelper };
