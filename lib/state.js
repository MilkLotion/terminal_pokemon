...
function pidsUpTo(chain, termPid) {
  if (!termPid) return new Set(chain); // 터미널 번호를 모름 — pkmon 없이 실행된 경우 (npm start)
...
//   claude 안의 !pkmon   pkmon → bash… → claude → bash(터미널) → Code                  host=claude
//   codex 안의 !pkmon    pkmon → pwsh → codex.exe → node(codex.js) → pwsh(터미널)      host=node(codex.js)
//   셸에서 바로 pkmon    pkmon → bash(터미널) → Code                                   host=null  term=bash(터미널)
...
//                 반응하지 않게 한다. CLI 상태는 그 CLI 안에서 !pkmon 으로 띄운 펫만 따른다
// 둘 다 없으면(pkmon 없이 npm start 로 직접 띄움) 내 터미널 셸이 조상에 있는 기록을 고른다
