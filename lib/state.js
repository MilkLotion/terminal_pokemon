// 펫(main.js)과 진단 도구(bin/pokebuddy-status)가 똑같은 판정을 쓰도록 공통 부분만 모은 곳
...
// 동반자는 폴링마다 한 번 읽어 호스트 판정(lib/follow hostOf)과 상태 판정(stateFor)에 같이 쓴다
...
function stateFor(records, pids) {
  const want = pids instanceof Set ? pids : new Set(pids || []);
