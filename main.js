...
// 판정 로직은 진단 도구(bin/pokebuddy-status)와 같은 것을 쓴다 — 두 벌이 되면 진단이 거짓말을 한다
...
function helperCommand() {
  ...
  const override = process.env.POKEBUDDY_WINBOUNDS;
...
// 못 잡은 펫은 가끔 다시 잡아 본다 — 잡고 있던 펫이 내려지면(pokebuddy stop eevee) 남은 펫이 이어받는다
...
app.whenReady().then(async () => {
  ...
  if (!art) {
    ...
    const sheet = settings.spritePath(config) || "저장소 모름 (POKEBUDDY_SOURCE 로 codex-pokepets 경로를 주면 art=sheet 를 쓸 수 있다)";
    process.stderr.write(`펫 그림을 찾을 수 없음: ${config.slug}\n  스프라이트시트: ${sheet}\n`);
    // 펫의 출력은 평소 버려진다 — pokebuddy 명령과 pokebuddy status 가 읽을 수 있게 이유를 남긴다
    reportFailure(
      config.art === "sheet" && !config.source
        ? "art=sheet 는 codex-pokepets 저장소 경로(POKEBUDDY_SOURCE)가 필요하다"
        : `${config.slug} 그림을 받지 못함 — 네트워크(프록시)를 확인하거나 다른 펫 이름으로 시도`,
    );
    app.exit(3); // 실패로 끝낸다 — pokebuddy 가 종료 코드를 보고 "펫이 뜨지 못함"을 알린다
    return;
  }
  clearFailure();
  createWindow();
